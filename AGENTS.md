...
keychain van het bouwende account te staan; `werkDV` draait enkel de centrale app.

**Deployen naar beide accounts:**
```bash
./scripts/deploy.sh          # Release build, signt + verplaatst naar /Applications/Loop.app
...
Bij iedere opdracht om Loop te installeren, deployen of voor beide accounts beschikbaar te maken:

1. Gebruik standaard `./scripts/deploy.sh` zonder `CONFIG`, zodat de actuele branch als Release-build naar
   `/Applications/Loop.app` gaat. Maak geen tweede kopie in een gebruikersmap; `nhop` en `werkDV` draaien
   allebei exact deze centrale app.
2. Verifieer eerst de centrale installatie met
   `codesign --verify --deep --strict --verbose=2 /Applications/Loop.app` en controleer dat bundle-ID
   `com.nielshop.Loop` en authority
   `Loop Self-Signed` zijn.
3. Inventariseer daarna alle resterende `Loop.app`-bundles in `/Applications`, `/Users/Shared`, de
   Applications-mappen van beide accounts en Loop-buildproducten in Xcode DerivedData. Verwijder uitsluitend
   exact geïdentificeerde dubbele buildartefacten nadat de centrale installatie geldig en gestart is.
4. Gebruik Spotlight als tweede, systeemwijde controle:
   `mdfind 'kMDItemCFBundleIdentifier == "com.nielshop.Loop"'`. Klaar betekent dat dit uitsluitend
   `/Applications/Loop.app` teruggeeft en dat een draaiend Loop-proces vanaf datzelfde pad komt.
5. Controleer dat `/Applications/Loop.app` en `Contents/MacOS/Loop` voor `werkDV` leesbaar en uitvoerbaar
   zijn. Accessibility en autostart blijven per account; rapporteer daarom expliciet wanneer `werkDV` Loop
   nog eenmalig moet aanvinken in Systeeminstellingen.
